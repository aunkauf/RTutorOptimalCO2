...
**Task:**
For this task the code is give, so just press check and get an insight to the data.

```{r "2_Exempted_Firms__14"}
...
```{r "2_Exempted_Firms__19",optional=TRUE}
...
**Task:**
Use the `group_by()` command in order to group the dataset `Ex1.1` by the variable `xquad`. Save the result in the variable `classification`.

```{r "2_1_Graphical_approa__7"}
# Enter your code here
```
...
Save it all in the data set `Ex1.1`.

```{r "2_1_Graphical_approa__8"}
# Enter your code here
```

So let's have a fist look at what we calculated until now

```{r "2_1_Graphical_approa__9"}
Ex1.1
```
...
```{r "2_1_Graphical_approa__11",optional=TRUE}
# Enter your command here
```
...
```{r "3_Vulnerability_scor__2"}
Dat=filter(Dat,is.na(unique)==FALSE,is.na(fimpact_score_clean)==FALSE)
```

# Overall characteristics

We want to begin by looking up some characteristics of the firms we interviewed. For this task we want to use the `stargazer`package which allows us to design nice tables out of data and regressions. If you want more information on the `stargazer` package check the info box below.
...
stargazer(characteristics, type="text",style="aer",
...
```{r "3_Vulnerability_scor__15",optional=TRUE}
# Enter your command here
```
...
**Task:**
Divide the dataset by the value of `sum.allo` and multiply all by $100$ to get the shares in percentage. Afterwards print it out.

```{r "5_Exploring_the_Resu__3"}
# Enter your code here
```

So let's interpret those shares. The first three variables are based on the risk of relocation associated with grandfathering and a firm-level optimization of the free permits. To keep the same level of risk it would be enough to allocate between $14.3$ and $24.5$ percent of the permits given out under grandfathering for free to the firms, depending on our assumptions about the weight of each risk (job loss and carbon leakage), which would be a huge increase in efficiency. 

Now to the other 3 variables: In the benchmarking scenario around $52.3$ percent of the permits that are given out in the grandfathering scenario are given for free. But this shortage raises the overall risk in this scenario and therefore pushes our improvements even further. With the higher threshold for risk we can reduce the amount of free permits to between $1.6$ to $13.0$ percent of the free permits used in the grandfathering scenario.
...
**Task:**
Calculate the risk arise through optimising across firms and across sectors in the same fashion as above. Be aware that these variables changed their names in the dataset `optimal`:
- `emp` is now `emp.x`
...
display("Share of employment at risk")
display("Reference scenario: Benchmarking")
emp.bench
```
...
display("Share of emissions at risk")
display("Reference scenario: Grandfathering")
co2.allo
display("Reference scenario: Benchmarking")
co2.bench
```
...
```{r "5_Exploring_the_Resu__13",optional=TRUE}
# Enter your command here
```

...
#< info "summarize()"
With the `summarize()` command we are able to use aggregate functions very effective especially on grouped data. If we want for example calculate the number of entries in each grouped specified in `MyGrouping` this can be done as follows:
...
Like to the previous part we want to find out if some sectors are more affected to carbon pricing than others. For this task our dataset contains the variable `mcetsdig`, which assigns different sectors to each firm. Let's see what characteristics `mcetsdig` does have.
...
Let's first formulate our expectations to this variables:
...
These results confirm the expectations we discussed above with a remarkable high significance (in the case of all firms) of at least at the 5% level. Although the statistical significance is for some variables lower if we only consider EU ETS firms, we can conclude that the VS is a quite consistent estimator for the vulnerability of a firm to carbon pricing.
...
- `opti_co2_allo_cost`: This time we fixed the risk of emission leakage ($\alpha$=1) at the level resulting from grandfathering
...
I hope you had fun solving this problem set. If so, make sure ou check 
